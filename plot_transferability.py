...
modelfolder = outfolder+'/Results_RFoob_CovMap_dfrac_subbas_Jul-08-2021/'
histfolder = outfolder+'/Histogram_distances_subbasins_sub9/'
catfolder = outfolder+'/Categorical_covariates_sub-basins9/'

#create subfolder for saving output
resultfolder = modelfolder+"/Correlation_plots_7/"
...
df_rmse = pd.read_csv(modelfolder +'/df_Performance_RMSE', index_col =0)
...
# dist_all['Intersection'] = pd.read_csv(histfolder+'df_Intersection_stdzn_A1', index_col =0)
...
def subplot_linear_com(dict_dist_in, dist_cat, df_perf, weight, weight_cat):
    ...
    for ax, metric in zip(axes.flatten(),dict_WD.keys()): #loop over metrics
        ...
        spearR, p = sc.spearmanr(x,y)

        ax.text(0.5, 0.9, r'$\rho_{P} = $'+ str('{:.2f}'.format(pearR)), horizontalalignment='center', verticalalignment='center', transform=ax.transAxes)
        # ax.text(0.5, 0.9, r'$\rho_{S} = $'+ str('{:.2f}'.format(spearR)), horizontalalignment='center', verticalalignment='center', transform=ax.transAxes)
        # ax.text(0.5, 0.7, r'$score = $'+ str('{:.2f}'.format(score)), horizontalalignment='center', verticalalignment='center', transform=ax.transAxes)
...
'''To do: in linear combination, you should compute R2 of each sub-basin from the 
remaining 9 points. perhaps save regression as output of linear function. 
make separate function that then plots R2 against R2.
